...
export class A5AlivedialModalComponent implements OnInit {
  ...
  makeAliveDialRequest() {
    this.http.get('https://api-v1.websitealive.com/alivedial/makecall/?format=json&callback=?', {
  ...
  makeCall(trigger: string) {
    ...
    this.httpOptions.data['phone_customer'] = userSubmittedNumber.internationalNumber.replace(/-|\s/g,"");
